...
deploy_to_vm() {
    ...
    # Step 4: Skip Docker image building in CI/CD context
    # Images are pre-built in the GitLab CI build stage and available in registry
    if [[ -n "${CI:-}" ]]; then
        log "INFO" "Running in CI/CD environment - using pre-built images from registry"
        log "INFO" "Images were built during the build stage and pushed to $DOCKER_REGISTRY"
...
deploy_locally() {
    ...
    # Step 2: Handle image building for local deployment
    if [[ -n "${CI:-}" ]]; then
        log "INFO" "Running in CI/CD environment - using pre-built images from registry"
        log "INFO" "Images were built during the build stage and pushed to $DOCKER_REGISTRY"
    elif [[ "$SKIP_BUILD" == "false" ]]; then
